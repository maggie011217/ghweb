	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if(method.equals("reg2")){//会员注册 reg2.jsp
			...
			if(username.trim().equals("admin")){
				request.setAttribute("username", username);
				request.setAttribute("message", "不能使用此用戶名！");
			...
			if(username.trim().equals("admin")){
				...
				if(flag == Constant.SUCCESS){
					request.setAttribute("username", username);
					request.setAttribute("message", "這個名字可以使用！");
					request.getRequestDispatcher("reg2.jsp").forward(request, response);
				}
				else if(flag == Constant.SAME_NAME){
					request.setAttribute("username", username);
		 ...
		 *********************************************/
			...
			if(str==null){
				...
				if(flag == Constant.SUCCESS){
						...
						request.setAttribute("message", "註冊成功！恭喜您成為"+sitename+"的會員！");
				...
				request.setAttribute("message", "此身分證已被註冊！");
				request.getRequestDispatcher("personreg.jsp").forward(request, response);
			}
			...
			if(info.trim().equals("error")){
				request.setAttribute("message", "資料錯誤！");
				request.getRequestDispatcher("login.jsp").forward(request, response);
			}
			else{
				request.setAttribute("message", "您的新密碼為"+info+"，請登入後修改！");
				request.getRequestDispatcher("login.jsp").forward(request, response);
			}
		}
		else{
