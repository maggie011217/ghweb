		...
		try{
			...
			if(method.equals("one")){//admin登录
				...
				if(username == null||username.trim().equals("")){
					request.setAttribute("message", "請輸入正確用戶名！");
					...
					ComBean cb=new ComBean();
					 ...
					 if(str!=null){  
						...
						request.getRequestDispatcher("admin/index.jsp").forward(request, response);  
						 ...
						 request.setAttribute("message", "登入資料錯誤！");
				...
				if(username2 == null){
					...
					switch (flag){
						...
						case Constant.PASSWORD_ERROR:
							request.setAttribute("message", "原始密碼錯誤！");
							request.getRequestDispatcher(sysdir+"/system/editpwd.jsp").forward(request, response);
							break;
				...
				if(username2 == null){
					...
					if(id == 1){
						request.setAttribute("message", "不能刪除原始帳號！");
						request.getRequestDispatcher(sysdir+"/system/user.jsp").forward(request, response);
					}
					else{
				...
				if(username2 == null){
					...
					if(check == null){
						request.setAttribute("message", "請選擇要刪除的紀錄！");
						...
						if(flag == Constant.SUCCESS){
							request.setAttribute("message", "刪除記錄成功！");
							request.getRequestDispatcher(sysdir+"/system/log.jsp").forward(request, response);
						}
						else{
							request.setAttribute("message", "系統維護中，請稍後再試！");
