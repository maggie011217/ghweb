...
function top(){
   	form3.action="<%=basePath%>ys.jsp?page=1";
    form3.submit();
}
function last(){
	...
	form3.action="<%=basePath%>ys.jsp?page="+form3.pageCount.value;
    	form3.submit();
	}
}
function pre(){
  ...
  if(page<=1){
    ...
    form3.action="<%=basePath%>ys.jsp?page="+(page-1);
    form3.submit();
  }
}
...
function bjump(){
	...
	if(form3.busjump.value>pageCount){//如果跳转文本框中输入的頁数超过最后一頁的数，则跳到最后一頁
	  ...
	  form3.submit();
		...
		form3.action="<%=basePath%>ys.jsp?page="+pageCount;
		form3.submit();
	}
}
...
var page=parseInt(form3.busjump.value);
   if(page==0){
      ...
      form3.action="<%=basePath%>ys.jsp?page="+page;
      form3.submit();
   }else{
      form3.action="<%=basePath%>ys.jsp?page="+page;
      form3.submit();
...
<BODY text=#000000 bgColor=#ffffff leftMargin=0 topMargin=0>
     ...
     <TABLE class=dragTable cellSpacing=0 cellPadding=0 width="100%" border=0>
		...
		List pagelist1 = cb.getMessage(Integer.parseInt(page1),"select * from ys order by id desc",10); //带进一个頁数，并返回该頁所要显示的信息 
        ...
        <TR>
		  ...
		  &nbsp;&nbsp;<a href="<%=basePath %>ysinfo.jsp?id=<%=allnew.get(0).toString()%>">點此查看詳情...</a>
		  </TD>
		</TR>
		<%}} %>
				</TBODY>
