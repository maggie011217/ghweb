...
function top(){
   	form3.action="<%=basePath%><%=dir %>/pb/index.jsp?page=1";
    form3.submit();
}
function last(){
	...
	form3.action="<%=basePath%><%=dir %>/pb/index.jsp?page="+form3.pageCount.value;
    	form3.submit();
	}
}
function pre(){
  ...
  if(page<=1){
    ...
    form3.action="<%=basePath%><%=dir %>/pb/index.jsp?page="+(page-1);
    form3.submit();
  }
}
...
function bjump(){
	...
	if(form3.busjump.value>pageCount){//如果跳转文本框中输入的頁数超过最后一頁的数，则跳到最后一頁
	  ...
	  form3.submit();
		...
		form3.action="<%=basePath%><%=dir %>/pb/index.jsp?page="+pageCount;
		form3.submit();
	}
}
...
var page=parseInt(form3.busjump.value);
   if(page==0){
      ...
      form3.action="<%=basePath%><%=dir %>/pb/index.jsp?page="+page;
      form3.submit();
   }else{
      form3.action="<%=basePath%><%=dir %>/pb/index.jsp?page="+page;
      form3.submit();
...
<table width='100%' cellspacing='1' cellpadding='3' bgcolor='#CCCCCC' class="tablewidth">
     ...
     <td width="5%" align="center">ID </td>
      <td  align="center">排班專家</td> 
      <td  align="center">排班服務</td> 
      <td  align="center">排班時間</td> 
    ...
    <%
			...
			int cou = cb.getMessageCount("select count(*) from pb");//得到信息总数			        
			...
			List pagelist1 = cb.getMessage(Integer.parseInt(page1),"select * from pb order by id desc",5);
	...
	<tr  class="trA" onMouseOver="this.className='trB'" onMouseOut="this.className='trA'"> 
      ...
      <td align="center" bgcolor="#FFFFFF"  style="border-bottom:1px dotted #ccc;"><a href="<%=basePath%>ComServlet?method=delpb&id=<%=pagelist2.get(0).toString()%>" >刪除</a></td>
    </tr>
